...
| `MENU`         | `1` Stats `2` Take Vote <br>`3`  Publish `4` Temperature | Menu screen. Admin chooses different options from here.                    |
| `VOTE`         | `1` Cand_A `2` Cand_B <br> `3` Cand_C `4` Cand_D | Voting screen for voters.                                                  |
| `CONFIRMATION` | `1` Yes `*` Back                     | Confirmation dialog for the voter.                                          |
| `STAT`         | `1` Vote counts <br>`*` Back `#` Exit      | Different statistics options are listed on this screen.                     |
